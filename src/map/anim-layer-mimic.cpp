        ...
        , coll_rect{}
    {
        ...
        sprite.setPosition(
            { (util::center(t_screenRect).x - (sprite.getGlobalBounds().size.x * 0.5f)),
              (util::bottom(t_screenRect) - (sprite.getGlobalBounds().size.y * 0.85f)) });

        coll_rect = util::scaleRectInPlaceCopy(sprite.getGlobalBounds(), 0.65f);
    }
        ...
        , m_anims{}
    {
        ...
        for (const sf::FloatRect & rect : t_rects)
        {
            m_anims.emplace_back(
                m_texture,
                textureRect(0),
                t_context.layout.calScaleBasedOnResolution(t_context, 1.8f),
                rect);
        }
    }
