#ifndef ANIM_LAYER_LAVA_HPP_INCLUDED
#define ANIM_LAYER_LAVA_HPP_INCLUDED
//
// anim-layer-lava.hpp
//
#include "map/tile-layer.hpp"
#include "subsystem/harm-collision-manager.hpp"
#include "subsystem/harm.hpp"

#include <vector>

#include <SFML/Graphics/Rect.hpp>
#include <SFML/Graphics/RenderStates.hpp>
#include <SFML/Graphics/Sprite.hpp>
#include <SFML/Graphics/Texture.hpp>

namespace sf
{
    class RenderTarget;
} // namespace sf

namespace bramblefore
{

    struct Context;

    //

    class LavaAnimationLayer final
        : public ITileLayer
        , public IHarmCollisionOwner
    {
      public:
        LavaAnimationLayer(const Context & t_context, const std::vector<sf::FloatRect> & t_rects);
        virtual ~LavaAnimationLayer() final;

        void draw(const Context & t_context, sf::RenderTarget & t_target, sf::RenderStates t_states)
            const final;

        void move(const Context & t_context, const sf::Vector2f & t_move) final;
        void dumpInfo() const final;
        void update(const Context & t_context, const float t_frameTimeSec) final;

        inline void appendVerts(const Context &, const sf::Vector2f &, const sf::Vector2f &) final
        {}

        const Harm avatarCollide(const Context & t_context, const sf::FloatRect & t_avatarRect) final;

      private:
        [[nodiscard]] std::size_t frameCount() const noexcept;
        [[nodiscard]] const sf::IntRect textureRect(const std::size_t t_frame) const noexcept;

      private:
        sf::Texture m_texture;
        std::vector<sf::Sprite> m_sprites;
        float m_elapsedTimeSec;
        float m_timePerFrameSec;
        std::size_t m_frameIndex;
    };

} // namespace bramblefore

#endif // ANIM_LAYER_ACID_HPP_INCLUDED
